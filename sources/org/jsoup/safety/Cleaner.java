...
public class Cleaner {
    ...
    public boolean isValid(Document dirtyDocument) {
        Validate.notNull(dirtyDocument);
        if (copySafeNodes(dirtyDocument.body(), Document.createShell(dirtyDocument.baseUri()).body()) == 0) {
            if (dirtyDocument.head().childNodes().size() == 0) {
                return true;
            }
        }
        return false;
    ...
    public boolean isValidBodyHtml(String bodyHtml) {
        ...
        dirty.body().insertChildren(0, Parser.parseFragment(bodyHtml, dirty.body(), "", errorList));
        if (copySafeNodes(dirty.body(), clean.body()) == 0 && errorList.size() == 0) {
            return true;
        }
        return false;
    }
    ...
    private int copySafeNodes(Element source, Element dest) {
        NodeVisitor cleaningVisitor = new CleaningVisitor(source, dest);
    ...
    private ElementMeta createSafeElement(Element sourceEl) {
        ...
        int numDiscarded = 0;
        Iterator it = sourceEl.attributes().iterator();
        while (it.hasNext()) {
            Attribute sourceAttr = (Attribute) it.next();

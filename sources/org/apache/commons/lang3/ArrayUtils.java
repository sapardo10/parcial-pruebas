...
public class ArrayUtils {
    ...
    static class C11521 implements Comparator<T> {
        C11521() {
        }
    ...
    public static <T> boolean isSorted(T[] r7, java.util.Comparator<T> r8) {
        /* JADX: method processing error */
/*
Error: jadx.core.utils.exceptions.JadxRuntimeException: Can't find immediate dominator for block B:17:0x0029 in {5, 10, 11, 12, 14, 16} preds:[]
...
*/
    ...
    L_0x0021:
        ...
        r1 = "Comparator should not be null.";
        r0.<init>(r1);
        throw r0;
        return;
        */
        throw new UnsupportedOperationException("Method not decompiled: org.apache.commons.lang3.ArrayUtils.isSorted(java.lang.Object[], java.util.Comparator):boolean");
    }
    ...
    public static Map<Object, Object> toMap(Object[] array) {
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            if (object instanceof Entry) {
                ...
                stringBuilder2.append("Array element ");
    ...
    public static int[] removeElements(int[] array, int... values) {
        if (!isEmpty(array)) {
            if (!isEmpty(values)) {
                ...
                for (len$ = 0; len$ < array.length; len$++) {
                    i$ = array[len$];
                    MutableInt count2 = (MutableInt) occurrences.get(Integer.valueOf(i$));
                    if (count2 != null) {
                        if (count2.decrementAndGet() == 0) {
                            occurrences.remove(Integer.valueOf(i$));
                        }
                        toRemove.set(len$);
                    }
                }
    ...
    static Object removeAll(Object array, BitSet indices) {
        ...
        int destIndex = 0;
        while (true) {
            nextSetBit = indices.nextSetBit(srcIndex);
            int set = nextSetBit;
            if (nextSetBit == -1) {
                break;
            }
            nextSetBit = set - srcIndex;
            if (nextSetBit > 0) {
                System.arraycopy(array, srcIndex, result, destIndex, nextSetBit);
                destIndex += nextSetBit;
            }
            srcIndex = indices.nextClearBit(set);
        }
        nextSetBit = srcLength - srcIndex;
        if (nextSetBit > 0) {
            System.arraycopy(array, srcIndex, result, destIndex, nextSetBit);
        }
        return result;
    ...
    public static <T extends Comparable<? super T>> boolean isSorted(T[] array) {
        return isSorted(array, new C11521());
    }
    ...
    public static <T> T[] removeAllOccurences(T[] array, T element) {
        ...
        while (true) {
            ...
            index = indexOf;
            if (indexOf == -1) {
                return removeAll((Object[]) array, Arrays.copyOf(indices, count));
            }
            indexOf = count + 1;
            indices[count] = index;
            count = indexOf;
    ...
    public static <T> T[] insert(int index, T[] array, T... values) {
        ...
        if (values != null) {
            if (values.length != 0) {
                ...
                Object[] result = (Object[]) Array.newInstance(array.getClass().getComponentType(), array.length + values.length);
                System.arraycopy(values, 0, result, index, values.length);
                if (index > 0) {
                    System.arraycopy(array, 0, result, 0, index);
                }

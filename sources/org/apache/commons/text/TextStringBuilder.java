...
public class TextStringBuilder implements CharSequence, Appendable, Serializable, Builder<String> {
    ...
    class StrBuilderReader extends Reader {
        ...
        public int read(char[] b, int off, int len) {
            if (off < 0 || len < 0 || off > b.length || off + len > b.length || off + len < 0) {
                ...
                TextStringBuilder textStringBuilder = TextStringBuilder.this;
                int i = this.pos;
                textStringBuilder.getChars(i, i + len, b, off);
                this.pos += len;
                return len;
            }
        }
    ...
    public org.apache.commons.text.TextStringBuilder setLength(int r6) {
...
*/
    ...
    L_0x0021:
        ...
        return;
        */
        throw new UnsupportedOperationException("Method not decompiled: org.apache.commons.text.TextStringBuilder.setLength(int):org.apache.commons.text.TextStringBuilder");
    }
    ...
    public TextStringBuilder insert(int index, boolean value) {
        ...
        if (value) {
            ensureCapacity(this.size + TRUE_STRING_SIZE);
            obj = this.buffer;
            System.arraycopy(obj, index, obj, TRUE_STRING_SIZE + index, this.size - index);
            cArr = this.buffer;
            i = index + 1;
            cArr[index] = 't';
            index = i + 1;
            ...
            ensureCapacity(this.size + FALSE_STRING_SIZE);
            obj = this.buffer;
            System.arraycopy(obj, index, obj, FALSE_STRING_SIZE + index, this.size - index);
            cArr = this.buffer;
            i = index + 1;
            cArr[index] = 'f';
            index = i + 1;

...
public final class GzipSource implements Source {
    ...
    public long read(Buffer sink, long byteCount) throws IOException {
        ...
        if (j < 0) {
            ...
            if (gzipSource.section == 1) {
                ...
                long result = gzipSource.inflaterSource.read(sink, j);
                if (result != -1) {
    ...
    private void consumeHeader() throws IOException {
        ...
        if (((flags >> 3) & 1) == 1) {
            long index = r6.source.indexOf(SECTION_HEADER);
            if (index != -1) {
                if (fhcrc) {
        ...
        if (((flags >> 4) & 1) == 1) {
            long index2 = r6.source.indexOf(SECTION_HEADER);
            if (index2 != -1) {
                if (fhcrc) {
                    updateCrc(r6.source.buffer(), 0, index2 + 1);
                }
                r6.source.skip(1 + index2);
            } else {
                throw new EOFException();
            }
        }

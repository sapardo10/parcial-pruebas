...
public final class ProgressionUtilKt {
    ...
    public static final int getProgressionLastElement(int start, int end, int step) {
        ...
        if (step > 0) {
            if (start < end) {
                differenceModulo = end - differenceModulo(end, start, step);
                return differenceModulo;
            }
        } else if (step >= 0) {
            throw new IllegalArgumentException("Step is zero.");
        } else if (start > end) {
            differenceModulo = differenceModulo(start, end, -step) + end;
            return differenceModulo;
        }
        differenceModulo = end;
        return differenceModulo;
    }

    @PublishedApi
    public static final long getProgressionLastElement(long start, long end, long step) {
        long differenceModulo;
        if (step > 0) {
            if (start < end) {
                differenceModulo = end - differenceModulo(end, start, step);
                return differenceModulo;

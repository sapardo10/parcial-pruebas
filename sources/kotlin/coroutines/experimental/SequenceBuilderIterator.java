...
final class SequenceBuilderIterator<T> extends SequenceBuilder<T> implements Iterator<T>, Continuation<Unit>, KMappedMarker {
    ...
    private final Throwable exceptionalState() {
        ...
        switch (this.state) {
            ...
            default:
                ...
                noSuchElementException = new IllegalStateException(stringBuilder.toString());
                break;
        }
    ...
    @Nullable
    public Object yield(T value, @NotNull Continuation<? super Unit> c) {
        this.nextValue = value;
        this.state = 3;
        setNextStep(CoroutineIntrinsics.normalizeContinuation(c));
        return IntrinsicsKt__IntrinsicsJvmKt.getCOROUTINE_SUSPENDED();
    }

    @Nullable
    public Object yieldAll(@NotNull Iterator<? extends T> iterator, @NotNull Continuation<? super Unit> c) {
        if (!iterator.hasNext()) {

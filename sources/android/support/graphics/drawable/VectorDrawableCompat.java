...
public class VectorDrawableCompat extends VectorDrawableCommon {
    ...
    private static class VPathRenderer {
        ...
        private void drawGroupTree(VGroup currentGroup, Matrix currentMatrix, Canvas canvas, int w, int h, ColorFilter filter) {
            ...
            for (int i = 0; i < vGroup.mChildren.size(); i++) {
                VGroup child = vGroup.mChildren.get(i);
    ...
    private static class VectorDrawableCompatState extends ConstantState {
        ...
        public boolean canReuseBitmap(int width, int height) {
            if (width == this.mCachedBitmap.getWidth()) {
                if (height == this.mCachedBitmap.getHeight()) {
                    return true;
                }
            }
            return false;
        ...
        public boolean canReuseCache() {
            if (!this.mCacheDirty && this.mCachedTint == this.mTint && this.mCachedTintMode == this.mTintMode && this.mCachedAutoMirrored == this.mAutoMirrored) {
                if (this.mCachedRootAlpha == this.mVPathRenderer.getRootAlpha()) {
                    return true;
                }
            }
            return false;
...
/*
Error: jadx.core.utils.exceptions.JadxRuntimeException: Can't find immediate dominator for block B:43:0x00f4 in {5, 6, 13, 14, 15, 20, 21, 22, 27, 28, 29, 30, 31, 35, 36, 37, 38, 40, 42} preds:[]
...
*/
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.mTmpBounds.width() > 0) {
            if (this.mTmpBounds.height() > 0) {
                ...
                if (canvasSkewX == 0.0f) {
                    if (canvasSkewY == 0.0f) {
                        scaledHeight = (int) (((float) this.mTmpBounds.height()) * canvasScaleY);
                        scaledWidth = Math.min(2048, (int) (((float) this.mTmpBounds.width()) * canvasScaleX));
    ...
    public boolean isStateful() {
        ...
        if (!super.isStateful()) {
            ...
            if (vectorDrawableCompatState != null && vectorDrawableCompatState.mTint != null) {

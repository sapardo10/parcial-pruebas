...
/*
Error: jadx.core.utils.exceptions.JadxRuntimeException: Can't find immediate dominator for block B:17:0x0035 in {6, 7, 8, 12, 14, 16} preds:[]
...
*/
    ...
    private static PropertyValuesHolder getPVH(TypedArray styledAttributes, int valueType, int valueFromId, int valueToId, String propertyName) {
        ...
        if (i3 == 4) {
            if (hasFrom) {
                if (!isColorType(fromType)) {
                }
                valueType2 = 3;
            }
            if (hasTo && isColorType(toType)) {
                valueType2 = 3;
            } else {
                valueType2 = 0;
            }
    ...
    private static void parseAnimatorFromTypeArray(ValueAnimator anim, TypedArray arrayAnimator, TypedArray arrayObjectAnimator, float pixelSize, XmlPullParser parser) {
        ...
        if (TypedArrayUtils.hasAttribute(parser, "valueFrom")) {
            if (TypedArrayUtils.hasAttribute(parser, "valueTo")) {
                if (valueType == 4) {
    ...
    private static void setupPathMotion(Path path, ObjectAnimator oa, float precision, String propertyXName, String propertyYName) {
        ...
        while (i < numPoints) {
            ...
            if (contourIndex + 1 < contourLengths.size()) {
                if (currentDistance > ((Float) contourLengths.get(contourIndex + 1)).floatValue()) {

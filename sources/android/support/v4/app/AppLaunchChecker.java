    ...
    public static void onActivityCreate(@NonNull Activity activity) {
        ...
        if (!sp.getBoolean(KEY_STARTED_FROM_LAUNCHER, false)) {
            ...
            if (launchIntent != null) {
                if ("android.intent.action.MAIN".equals(launchIntent.getAction())) {
                    if (!launchIntent.hasCategory("android.intent.category.LAUNCHER")) {
                        if (launchIntent.hasCategory(IntentCompat.CATEGORY_LEANBACK_LAUNCHER)) {
                        }
                    }

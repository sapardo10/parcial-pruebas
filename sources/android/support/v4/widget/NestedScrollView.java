...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild2, ScrollingView {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.mFillViewport && MeasureSpec.getMode(heightMeasureSpec) != 0) {
            if (getChildCount() > 0) {
    ...
    public boolean executeKeyEvent(@NonNull KeyEvent event) {
            ...
            if (event.getAction() == 0) {
                ...
                if (keyCode != 62) {
                    switch (keyCode) {
                        ...
                        case 20:
                            if (!event.isAltPressed()) {
                                canScroll = arrowScroll(TsExtractor.TS_STREAM_TYPE_HDMV_DTS);
                                break;
                            }
                            canScroll = fullScroll(TsExtractor.TS_STREAM_TYPE_HDMV_DTS);
                            break;
                        default:
                            break;
                    }
        ...
        if (nextFocused != null && nextFocused != this) {
            if (nextFocused.requestFocus(TsExtractor.TS_STREAM_TYPE_HDMV_DTS)) {
                z = true;
                return z;
            }
        }
                ...
                case 2:
                    ...
                    if (i != -1) {
                        ...
                        if (pointerIndex != -1) {
                            ...
                            if (Math.abs(y - nestedScrollView.mLastMotionY) > nestedScrollView.mTouchSlop) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (actionMasked) {
            ...
            case 2:
                ...
                break;
            case 3:
                if (nestedScrollView.mIsBeingDragged && getChildCount() > 0) {
                    if (nestedScrollView.mScroller.springBack(getScrollX(), getScrollY(), 0, 0, 0, getScrollRange())) {
                        ViewCompat.postInvalidateOnAnimation(this);
                    }
                }
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (top < viewBottom && viewTop < bottom) {
                ...
                if (focusCandidate == null) {
                    ...
                    if (topFocus) {
                        if (viewTop < focusCandidate.getTop()) {
                    ...
                    if (!topFocus) {
                        if (viewBottom <= focusCandidate.getBottom()) {
                        }
                        viewIsCloserToBoundary = true;
                        if (foundFullyContainedFocusable) {
    ...
    public boolean pageScroll(int direction) {
        boolean down = direction == TsExtractor.TS_STREAM_TYPE_HDMV_DTS;
    ...
    public boolean arrowScroll(int direction) {
        ...
        if (nextFocused == null || !isWithinDeltaOfScreen(nextFocused, maxJump, getHeight())) {
            ...
            doScrollY(direction == TsExtractor.TS_STREAM_TYPE_HDMV_DTS ? scrollDelta : -scrollDelta);
        } else {
    ...
    private boolean isWithinDeltaOfScreen(View descendant, int delta, int height) {
        ...
        if (this.mTempRect.bottom + delta >= getScrollY()) {
            if (this.mTempRect.top - delta <= getScrollY() + height) {
                return true;
            }
        }
        return false;

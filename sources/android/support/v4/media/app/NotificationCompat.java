...
import android.support.mediacompat.C0078R;
...
public class NotificationCompat {
    ...
    public static class MediaStyle extends Style {
        ...
        RemoteViews generateContentView() {
            ...
            view.removeAllViews(C0078R.id.media_actions);
            if (numActionsInCompact > 0) {
                int i = 0;
                while (i < numActionsInCompact) {
                    if (i < numActions) {
                        view.addView(C0078R.id.media_actions, generateMediaActionButton((Action) this.mBuilder.mActions.get(this.mActionsToShowInCompact[i])));
                        i++;
                    } else {
            ...
            if (this.mShowCancelButton) {
                view.setViewVisibility(C0078R.id.end_padder, 8);
                view.setViewVisibility(C0078R.id.cancel_action, 0);
                view.setOnClickPendingIntent(C0078R.id.cancel_action, this.mCancelButtonIntent);
                view.setInt(C0078R.id.cancel_action, "setAlpha", this.mBuilder.mContext.getResources().getInteger(C0078R.integer.cancel_button_image_alpha));
            } else {
                view.setViewVisibility(C0078R.id.end_padder, 0);
                view.setViewVisibility(C0078R.id.cancel_action, 8);
            }
            return view;
        }
        ...
        private RemoteViews generateMediaActionButton(Action action) {
            ...
            RemoteViews button = new RemoteViews(this.mBuilder.mContext.getPackageName(), C0078R.layout.notification_media_action);
            button.setImageViewResource(C0078R.id.action0, action.getIcon());
            if (!tombstone) {
                button.setOnClickPendingIntent(C0078R.id.action0, action.getActionIntent());
            }
            if (VERSION.SDK_INT >= 15) {
                button.setContentDescription(C0078R.id.action0, action.getTitle());
            }
            return button;
        }
        ...
        RemoteViews generateBigContentView() {
            ...
            big.removeAllViews(C0078R.id.media_actions);
            if (actionCount > 0) {
                for (int i = 0; i < actionCount; i++) {
                    big.addView(C0078R.id.media_actions, generateMediaActionButton((Action) this.mBuilder.mActions.get(i)));
                }
            }
            if (this.mShowCancelButton) {
                big.setViewVisibility(C0078R.id.cancel_action, 0);
                big.setInt(C0078R.id.cancel_action, "setAlpha", this.mBuilder.mContext.getResources().getInteger(C0078R.integer.cancel_button_image_alpha));
                big.setOnClickPendingIntent(C0078R.id.cancel_action, this.mCancelButtonIntent);
            } else {
                big.setViewVisibility(C0078R.id.cancel_action, 8);
            }
            return big;
        }

        int getBigContentViewLayoutResource(int actionCount) {
            return actionCount <= 3 ? C0078R.layout.notification_template_big_media_narrow : C0078R.layout.notification_template_big_media;
        }
    }
    ...
    public static class DecoratedMediaCustomViewStyle extends MediaStyle {
        ...
        int getBigContentViewLayoutResource(int actionCount) {
            return actionCount <= 3 ? C0078R.layout.notification_template_big_media_narrow_custom : C0078R.layout.notification_template_big_media_custom;
        }
        ...
        private void setBackgroundColor(RemoteViews views) {
            ...
            if (this.mBuilder.getColor() != 0) {
                ...
                color = this.mBuilder.mContext.getResources().getColor(C0078R.color.notification_material_background_media_default_color);
            }
            views.setInt(C0078R.id.status_bar_latest_event_content, "setBackgroundColor", color);
        }
    }

...
public class AppCompatActivity extends FragmentActivity implements AppCompatCallback, TaskStackBuilder$SupportParentable, ActionBarDrawerToggle$DelegateProvider {
    ...
    public final boolean onMenuItemSelected(int featureId, MenuItem item) {
        ...
        if (item.getItemId() == 16908332 && ab != null) {
            if ((ab.getDisplayOptions() & 4) != 0) {
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (keyCode == 82 && actionBar != null) {
            if (actionBar.onMenuKeyEvent(event)) {
                return true;
            }
        }
    ...
    private boolean performMenuItemShortcut(int keycode, KeyEvent event) {
        if (VERSION.SDK_INT < 26 && !event.isCtrlPressed()) {
                ...
                if (event.getRepeatCount() == 0) {
                    if (!KeyEvent.isModifierKey(event.getKeyCode())) {
                        ...
                        if (currentWindow != null && currentWindow.getDecorView() != null) {
                            if (currentWindow.getDecorView().dispatchKeyShortcutEvent(event)) {
                                return true;
                            }
                        }
                    }

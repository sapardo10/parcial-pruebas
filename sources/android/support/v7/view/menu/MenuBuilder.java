    ...
    private static int getOrdering(int categoryOrder) {
        ...
        if (index >= 0) {
            ...
            if (index < iArr.length) {
                return (iArr[index] << 16) | (SupportMenu.USER_MASK & categoryOrder);
            }
        }
    ...
    private void setShortcutsVisibleInner(boolean shortcutsVisible) {
        ...
        if (shortcutsVisible) {
            if (this.mResources.getConfiguration().keyboard != 1) {
                if (this.mResources.getBoolean(C0286R.bool.abc_config_showMenuShortcutsWhenKeyboardPresent)) {
    ...
    void findItemsWithShortcutForKey(List<MenuItemImpl> items, int keyCode, KeyEvent event) {
        ...
        if (keyEvent.getKeyData(possibleChars) || i == 67) {
            ...
            for (int i2 = 0; i2 < N; i2++) {
                ...
                if (((modifierState & SupportMenu.SUPPORTED_MODIFIERS_MASK) == (SupportMenu.SUPPORTED_MODIFIERS_MASK & (qwerty ? item.getAlphabeticModifiers() : item.getNumericModifiers()))) && shortcutChar != '\u0000' && (shortcutChar == possibleChars.meta[0] || shortcutChar == possibleChars.meta[2] || (qwerty && shortcutChar == '\b' && i == 67))) {
                    if (item.isEnabled()) {
                        list.add(item);
                    }
                }

...
public class RemotePlaybackClient {
                    ...
                    if (sessionIdResult != null) {
                        ...
                        try {
                            ...
                            if (intent.getAction().equals(MediaControlIntent.ACTION_END_SESSION)) {
                                if (sessionIdResult.equals(RemotePlaybackClient.this.mSessionId)) {
    ...
    private void detectFeatures() {
        boolean z;
        boolean z2 = true;
        if (routeSupportsAction(MediaControlIntent.ACTION_PLAY)) {
            if (routeSupportsAction(MediaControlIntent.ACTION_SEEK)) {
                if (routeSupportsAction(MediaControlIntent.ACTION_GET_STATUS)) {
                    if (routeSupportsAction(MediaControlIntent.ACTION_PAUSE)) {
                        if (routeSupportsAction(MediaControlIntent.ACTION_RESUME)) {
                            if (routeSupportsAction(MediaControlIntent.ACTION_STOP)) {
                                z = true;
                                this.mRouteSupportsRemotePlayback = z;
                                if (this.mRouteSupportsRemotePlayback) {
                                    if (routeSupportsAction(MediaControlIntent.ACTION_ENQUEUE)) {
                                        if (routeSupportsAction(MediaControlIntent.ACTION_REMOVE)) {
                                            z = true;
                                            this.mRouteSupportsQueuing = z;
                                            if (!this.mRouteSupportsRemotePlayback) {
                                                if (!routeSupportsAction(MediaControlIntent.ACTION_START_SESSION)) {
                                                    if (!routeSupportsAction(MediaControlIntent.ACTION_GET_SESSION_STATUS)) {
                                                        if (!routeSupportsAction(MediaControlIntent.ACTION_END_SESSION)) {

...
public class SortedList<T> {
    ...
    L_0x005a:
        ...
        java.lang.System.arraycopy(r2, r4, r6, r7, r5);
        r2 = r10.mNewDataStart;
        r2 = r2 + r5;
        r10.mNewDataStart = r2;
    L_0x006a:
        r2 = 0;
        r10.mOldData = r2;
    ...
    private void removeItemAtIndex(int index, boolean notify) {
        ...
        System.arraycopy(obj, index + 1, obj, index, (this.mSize - index) - 1);
        this.mSize--;
    ...
    public void updateItemAt(int index, T item) {
        ...
        if (existing != item) {
            if (this.mCallback.areContentsTheSame(existing, item)) {
                ...
                if (existing != item) {
                    if (this.mCallback.compare(existing, item) == 0) {
                        this.mData[index] = item;
                        if (contentsChanged) {
                            Callback callback = this.mCallback;
                            callback.onChanged(index, 1, callback.getChangePayload(existing, item));
        ...
        if (existing != item) {
            if (this.mCallback.compare(existing, item) == 0) {
                ...
                if (contentsChanged) {
                    ...
                    callback3.onChanged(index, 1, callback3.getChangePayload(existing, item));
                }
                return;
            }
        }
        if (contentsChanged) {
            ...
            callback22.onChanged(index, 1, callback22.getChangePayload(existing, item));
        }
        removeItemAtIndex(index, false);
        newIndex = add(item, false);
        if (index != newIndex) {
    ...
    public T get(int index) throws IndexOutOfBoundsException {
        ...
        if (objArr != null) {
            int i = this.mNewDataStart;
            if (index >= i) {
                return objArr[(index - i) + this.mOldDataStart];
            }
        }
    ...
    private void addToData(int index, T item) {
        ...
        if (index <= i) {
            Object obj = this.mData;
            if (i == obj.length) {
                Object[] newData = (Object[]) Array.newInstance(this.mTClass, obj.length + 10);

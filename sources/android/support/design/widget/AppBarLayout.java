...
public class AppBarLayout extends LinearLayout {
    ...
    public static class LayoutParams extends android.widget.LinearLayout.LayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            super(c, attrs);
            TypedArray a = c.obtainStyledAttributes(attrs, C0035R.styleable.AppBarLayout_Layout);
            this.mScrollFlags = a.getInt(C0035R.styleable.AppBarLayout_Layout_layout_scrollFlags, 0);
            if (a.hasValue(C0035R.styleable.AppBarLayout_Layout_layout_scrollInterpolator)) {
                this.mScrollInterpolator = AnimationUtils.loadInterpolator(c, a.getResourceId(C0035R.styleable.AppBarLayout_Layout_layout_scrollInterpolator, 0));
            }
            a.recycle();
        }
    ...
    class C08081 implements OnApplyWindowInsetsListener {
        C08081() {
        }
    ...
    public static class Behavior extends HeaderBehavior<AppBarLayout> {
        ...
        public boolean onStartNestedScroll(CoordinatorLayout parent, AppBarLayout child, View directTargetChild, View target, int nestedScrollAxes, int type) {
            ...
            if ((nestedScrollAxes & 2) != 0) {
                if (child.hasScrollableChildren()) {
                    if (parent.getHeight() - directTargetChild.getHeight() <= child.getHeight()) {
                        started = true;
                        if (started) {
        ...
        public boolean onLayoutChild(CoordinatorLayout parent, AppBarLayout abl, int layoutDirection) {
            ...
            View child = this.mOffsetToChildIndexOnLayout;
            int offset;
            if (child >= null && (pendingAction & 8) == 0) {
                child = abl.getChildAt(child);
        ...
        boolean canDragView(AppBarLayout view) {
            ...
            WeakReference weakReference = this.mLastNestedScrollingChildRef;
            boolean z = true;
            if (weakReference == null) {
                return true;
            }
            View scrollingView = (View) weakReference.get();
            if (scrollingView != null && scrollingView.isShown()) {
        ...
        private void updateAppBarLayoutDrawableState(CoordinatorLayout parent, AppBarLayout layout, int offset, int direction, boolean forceJump) {
            ...
            if (child != null) {
                ...
                if (VERSION.SDK_INT >= 11) {
                    if (!forceJump) {
                        if (changed) {
                            if (!shouldJumpElevationState(parent, layout)) {
                                return;
                            }
                        }
                    }
    ...
    public static class ScrollingViewBehavior extends HeaderScrollingViewBehavior {
        ...
        public ScrollingViewBehavior(Context context, AttributeSet attrs) {
            super(context, attrs);
            TypedArray a = context.obtainStyledAttributes(attrs, C0035R.styleable.ScrollingViewBehavior_Layout);
            setOverlayTop(a.getDimensionPixelSize(C0035R.styleable.ScrollingViewBehavior_Layout_behavior_overlapTop, 0));
            a.recycle();
        }
    ...
    public AppBarLayout(Context context, AttributeSet attrs) {
        ...
        if (VERSION.SDK_INT >= 21) {
            ...
            ViewUtilsLollipop.setStateListAnimatorFromAttrs(this, attrs, 0, C0035R.style.Widget_Design_AppBarLayout);
        }
        TypedArray a = context.obtainStyledAttributes(attrs, C0035R.styleable.AppBarLayout, 0, C0035R.style.Widget_Design_AppBarLayout);
        ViewCompat.setBackground(this, a.getDrawable(C0035R.styleable.AppBarLayout_android_background));
        if (a.hasValue(C0035R.styleable.AppBarLayout_expanded)) {
            setExpanded(a.getBoolean(C0035R.styleable.AppBarLayout_expanded, false), false, false);
        }
        if (VERSION.SDK_INT >= 21 && a.hasValue(C0035R.styleable.AppBarLayout_elevation)) {
            ViewUtilsLollipop.setDefaultAppBarLayoutStateListAnimator(this, (float) a.getDimensionPixelSize(C0035R.styleable.AppBarLayout_elevation, 0));
        }
        if (VERSION.SDK_INT >= 26) {
            if (a.hasValue(C0035R.styleable.AppBarLayout_android_keyboardNavigationCluster)) {
                setKeyboardNavigationCluster(a.getBoolean(C0035R.styleable.AppBarLayout_android_keyboardNavigationCluster, false));
            }
            if (a.hasValue(C0035R.styleable.AppBarLayout_android_touchscreenBlocksFocus)) {
                setTouchscreenBlocksFocus(a.getBoolean(C0035R.styleable.AppBarLayout_android_touchscreenBlocksFocus, false));
            }
        }
        a.recycle();
        ViewCompat.setOnApplyWindowInsetsListener(this, new C08081());
    }
    ...
    public final int getTotalScrollRange() {
        ...
        for (i2 = 0; i2 < z; i2++) {
            ...
            i += (lp.topMargin + childHeight) + lp.bottomMargin;
            if ((flags & 2) != 0) {
                i -= ViewCompat.getMinimumHeight(child);
                break;
            }
        }
        i2 = Math.max(0, i - getTopInset());
    ...
    int getDownNestedPreScrollRange() {
        ...
        for (i2 = getChildCount() - 1; i2 >= 0; i2--) {
            ...
            int flags = lp.mScrollFlags;
            if ((flags & 5) == 5) {
                i += lp.topMargin + lp.bottomMargin;
                if ((flags & 8) != 0) {
                    i += ViewCompat.getMinimumHeight(child);
                } else if ((flags & 2) != 0) {
                    i += childHeight - ViewCompat.getMinimumHeight(child);
                } else {
                    i += childHeight - getTopInset();
                }
            } else if (i > 0) {
                break;
            }
    ...
    int getDownNestedScrollRange() {
        ...
        for (i2 = 0; i2 < z; i2++) {
            ...
            i += childHeight;
            if ((flags & 2) != 0) {
                i -= ViewCompat.getMinimumHeight(child) + getTopInset();
                break;
            }
        }
        i2 = Math.max(0, i);
    ...
    protected int[] onCreateDrawableState(int extraSpace) {
        ...
        extraStates[0] = this.mCollapsible ? C0035R.attr.state_collapsible : -C0035R.attr.state_collapsible;
        int i = (this.mCollapsible && this.mCollapsed) ? C0035R.attr.state_collapsed : -C0035R.attr.state_collapsed;
        extraStates[1] = i;

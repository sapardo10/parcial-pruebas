...
public final class DtsUtil {
    ...
    public static boolean isSyncWord(int word) {
        if (!(word == SYNC_VALUE_BE || word == SYNC_VALUE_LE || word == SYNC_VALUE_14B_BE)) {
            if (word != SYNC_VALUE_14B_LE) {
                return false;
            }
        }
        return true;
    ...
    private static boolean isLittleEndianFrameHeader(byte[] frameHeader) {
        if (frameHeader[0] != FIRST_BYTE_LE) {
            if (frameHeader[0] != FIRST_BYTE_14B_LE) {
                return false;
            }
        }
        return true;

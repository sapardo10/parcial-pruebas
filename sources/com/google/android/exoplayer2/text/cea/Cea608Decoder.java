...
public final class Cea608Decoder extends CeaDecoder {
    ...
    private static final int[] SPECIAL_ES_FR_CHARACTER_SET = new int[]{193, 201, 211, 218, 220, 252, 8216, 161, 42, 39, 8212, 169, 8480, 8226, 8220, 8221, PsExtractor.AUDIO_STREAM, 194, 199, 200, 202, 203, 235, 206, 207, 239, 212, 217, 249, 219, 171, 187};
    ...
    private static class CueBuilder {
        ...
        public SpannableString buildSpannableString() {
            ...
            int nextColor = -1;
            for (int i = 0; i < r0.cueStyles.size(); i++) {
                CueStyle cueStyle = (CueStyle) r0.cueStyles.get(i);
                ...
                if (position != (i + 1 < r0.cueStyles.size() ? ((CueStyle) r0.cueStyles.get(i + 1)).start : length)) {
        ...
        public Cue build() {
            ...
            for (int i = 0; i < cueBuilder.rolledUpCaptions.size(); i++) {
            ...
            if (cueBuilder.captionMode == 2 && (Math.abs(startEndPaddingDelta) < 3 || endPadding < 0)) {
                position = 0.5f;
                positionAnchor = 1;
            } else if (cueBuilder.captionMode != 2 || startEndPaddingDelta <= 0) {
                positionAnchor = 0;
    ...
    private void setCaptionMode(int captionMode) {
        if (this.captionMode != captionMode) {
            ...
            this.captionMode = captionMode;
            resetCueBuilders();
            if (!(oldCaptionMode == 3 || captionMode == 1)) {
                if (captionMode != 0) {
                }
            }
            this.cues = Collections.emptyList();

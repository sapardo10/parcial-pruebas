...
public class CustomTypefaceSpan extends ReplacementSpan {
    ...
    public int getSize(Paint paint, CharSequence text, int start, int end, FontMetricsInt fm) {
        ...
        if (fm != null) {
            fm.descent = (int) (((float) TEXT_BOUNDS.height()) * (this.baselineAligned ? 0.0f : BASELINE_RATIO));
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
        ...
        if (this.rotate) {
            canvas.rotate((((float) (System.currentTimeMillis() - this.rotationStartTime)) / 2000.0f) * 360.0f, (((float) TEXT_BOUNDS.width()) / 2.0f) + x, (((float) y) - (((float) TEXT_BOUNDS.height()) / 2.0f)) + (((float) TEXT_BOUNDS.height()) * baselineRatio));
        }
        canvas.drawText(this.icon, x - ((float) TEXT_BOUNDS.left), ((float) (y - TEXT_BOUNDS.bottom)) + (((float) TEXT_BOUNDS.height()) * baselineRatio), paint);
        canvas.restore();
    }

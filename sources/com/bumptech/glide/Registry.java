...
public class Registry {
    ...
    public <Data> Registry register(@NonNull Class<Data> dataClass, @NonNull Encoder<Data> encoder) {
        return append((Class) dataClass, (Encoder) encoder);
    }
    ...
    private <Data, TResource, Transcode> List<DecodePath<Data, TResource, Transcode>> getDecodePaths(@NonNull Class<Data> dataClass, @NonNull Class<TResource> resourceClass, @NonNull Class<Transcode> transcodeClass) {
        ...
        ArrayList decodePaths = new ArrayList();
        for (Class<TResource> registeredResourceClass : this.decoderRegistry.getResourceClasses(cls, resourceClass)) {
            for (Class<Transcode> registeredTranscodeClass : r0.transcoderRegistry.getTranscodeClasses(registeredResourceClass, transcodeClass)) {
                decodePaths.add(new DecodePath(dataClass, registeredResourceClass, registeredTranscodeClass, r0.decoderRegistry.getDecoders(cls, registeredResourceClass), r0.transcoderRegistry.get(registeredResourceClass, registeredTranscodeClass), r0.throwableListPool));
            }
        }
        Class<Transcode> cls2 = transcodeClass;
        return decodePaths;
    }

...
public final class ObservableSkipLastTimed<T> extends AbstractObservableWithUpstream<T, T> {
    ...
    static final class SkipLastTimedObserver<T> extends AtomicInteger implements Observer<T>, Disposable {
        ...
        void drain() {
            ...
            if (getAndIncrement() == 0) {
                int missed = 1;
                Observer<? super T> a = skipLastTimedObserver.downstream;
                SpscLinkedArrayQueue<Object> q = skipLastTimedObserver.queue;
                boolean delayError = skipLastTimedObserver.delayError;
                TimeUnit unit = skipLastTimedObserver.unit;
                Scheduler scheduler = skipLastTimedObserver.scheduler;
                long time = skipLastTimedObserver.time;
                while (!skipLastTimedObserver.cancelled) {
                    boolean d = skipLastTimedObserver.done;
                    Long ts = (Long) q.peek();

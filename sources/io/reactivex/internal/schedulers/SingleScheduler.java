...
public final class SingleScheduler extends Scheduler {
    ...
    public void start() {
        ...
        while (true) {
            ScheduledExecutorService current = (ScheduledExecutorService) this.executor.get();
            if (current != SHUTDOWN) {
                break;
            }
            if (next == null) {
            ...
            if (this.executor.compareAndSet(current, next)) {
                return;
            }
        }
        if (next != null) {
    ...
    public void shutdown() {
        ...
        if (current != scheduledExecutorService) {
            current = (ScheduledExecutorService) this.executor.getAndSet(scheduledExecutorService);
            if (current != SHUTDOWN) {
                current.shutdownNow();
            }
        }
